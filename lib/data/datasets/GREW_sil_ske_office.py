...
class DataSet_GREW_sp_office(tordata.Dataset):
    ...
    def __load_seqs_into_list(self, dataset_root, smpl_root, subset, sub_test, lab, typ, vie, seqs_info_list, data_in_use):
        msg_mgr = get_msg_mgr()
        ...
        smpl_path = osp.join(smpl_root, subset, sub_test, *seq_info[:-2]) if sub_test == "probe" \
            else osp.join(smpl_root, subset, sub_test, *seq_info[:-1])
