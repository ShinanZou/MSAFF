...
class DataSet_GREW_sp_our(tordata.Dataset):
    ...
    def __load_seqs_into_list(self, dataset_root, smpl_root, lab, typ, vie, seqs_info_list, data_in_use):
        msg_mgr = get_msg_mgr()
        seq_info = [lab, typ, vie]
        seq_path = osp.join(dataset_root, *seq_info[:-1])
        smpl_path = osp.join(smpl_root, *seq_info[:-1])
