...
from data.datasets.dataset_multimodal import DataSet_multimodal
from data.datasets.GREW_office import DataSet_GREW_office
from data.datasets.GREW_our import DataSet_GREW_our
from data.datasets.GREW_sil_ske_our import DataSet_GREW_sp_our
from data.datasets.GREW_sil_ske_office import DataSet_GREW_sp_office
...
class BaseModel(MetaModel, nn.Module):
    ...
    def get_loader(self, data_cfg, train=True):
        ...
        try:
            if data_cfg['dataset_root']['smpl_root'] is not None and data_cfg['dataset_name'] in ['CASIA-Bps', 'OUMVLPps']:  # 骨骼多模态输入
                 dataset = DataSet_multimodal(data_cfg, train)
            elif data_cfg['dataset_root']['smpl_root'] is not None and "Gait3D" in data_cfg['dataset_name']:
                 dataset = DataSet_SMPLGait(data_cfg, train)
            elif data_cfg['dataset_root']['smpl_root'] is not None and "GREW-our" in data_cfg['dataset_name']:
                 dataset = DataSet_GREW_sp_our(data_cfg, train)
            elif data_cfg['dataset_root']['smpl_root'] is not None and "GREW-office" in data_cfg['dataset_name']:
                 dataset = DataSet_GREW_sp_office(data_cfg, train)
        except:
    ...
    def inputs_pretreament(self, inputs):
        ...
        try:
            ...
            if "ske" in self.cfgs['data_cfg']['dataset_root']:
                seqs_batch, labs_batch, typs_batch, vies_batch, seqL_batch = inputs

                requires_grad = bool(self.training)
                seqs = [np2var(np.asarray([fra for fra in seq]), requires_grad=requires_grad).float()
